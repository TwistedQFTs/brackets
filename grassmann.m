...
Grading[ Unevaluated[GD[a_, _]] ] := Grading[a] + 1;(*!Jingxiang: GD will always change the grading by 1!! So for the derivation w.r.t bosonic variables,
 it has to be done using the ordinary derivation!!*)
...
(*GD[ a_, b_, c_ ] := If[OrderedQ[b,c], GD[ GD[a, b], c ], 
                                      (-1)^Grading[b**c] GD[ GD[a,c], b ] ];*)(*original*)
GD[ a_, b_, c_ ] := If[OrderedQ[{b,c}], GD[ GD[a, b], c ], 
                                      (-1)^(Grading[b]*Grading[c]) GD[ GD[a,c], b ] ]; (*Jingxiang: my modification*)                       
...
GCollect[a_, b_List] := 
 Inner[
  ...
  (*
  Jingxiang: GCollect seems to be working improperly if b has bosonic variables. e.g. see 
  GCollect[\[Gamma][1]**\[Gamma][1],{\[Gamma][1]}] gives me 2\[Gamma][1]**\[Gamma][1]
